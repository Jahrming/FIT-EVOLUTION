...
type AceptacionClientProps = {
  initialPageError: string | null
  initialSede: SedeInfo | null
  sedeSlug: string
}

export default function AceptacionClient({
  initialPageError,
  initialSede,
  sedeSlug,
}: AceptacionClientProps) {
  const [currentStep, setCurrentStep] = useState(-1)
  const [sede, setSede] = useState<SedeInfo | null>(initialSede)
  const [isLoading, setIsLoading] = useState(!initialSede && !initialPageError)
  const [pageError, setPageError] = useState<string | null>(initialPageError)
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      const sedeData = initialSede || (await getSede(sedeSlug))
